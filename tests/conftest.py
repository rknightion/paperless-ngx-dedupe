...
def client(temp_db):
    ...
    test_app = FastAPI(
        title=settings.app_name, version=settings.version, lifespan=test_lifespan
    )
    ...
    test_app.include_router(
        duplicates.router, prefix="/api/v1/duplicates", tags=["duplicates"]
    )
    test_app.include_router(config.router, prefix="/api/v1/config", tags=["config"])
    test_app.include_router(
        processing.router, prefix="/api/v1/processing", tags=["processing"]
    )
