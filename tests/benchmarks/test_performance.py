...
class TestDeduplicationPerformance:
    ...
    def test_minhash_generation_performance(self, doc_count):
        ...
        assert docs_per_second > 50, f"MinHash generation too slow: {docs_per_second:.1f} docs/sec"
        assert memory_used < doc_count * 0.1, f"Memory usage too high: {memory_used:.1f} MB"
    ...
    def test_lsh_index_building_performance(self, doc_count):
        ...
        # Performance assertions
        assert docs_per_second > 20, f"LSH indexing too slow: {docs_per_second:.1f} docs/sec"
    ...
    def test_duplicate_finding_performance(self, doc_count):
        ...
        # Performance assertions
        assert docs_per_second > 10, f"Duplicate finding too slow: {docs_per_second:.1f} docs/sec"
        assert total_time < doc_count * 0.1, f"Time complexity seems O(n²): {total_time:.2f}s for {doc_count} docs"
    ...
    def test_memory_scaling(self):
        ...
        for count in doc_counts:
            ...
            print(f"Documents: {count}, Memory: {memory_used:.1f} MB, Per doc: {memory_used/count:.3f} MB")
            
            # Clear for next iteration
            self.service.minhashes.clear()
            self.service.lsh_index._hashtables = [dict() for _ in range(self.service.lsh_index.b)]
        ...
        for i in range(1, len(memory_usage)):
            ...
            assert ratio < doc_ratio * 1.5, f"Memory scaling too steep: {ratio:.2f}x for {doc_ratio:.2f}x docs"
...
class TestDatabasePerformance:
    ...
    def test_document_insertion_performance(self, db_session):
        ...
        assert docs_per_second > 100, f"Database insertion too slow: {docs_per_second:.1f} docs/sec"
...
class TestCachePerformance:
    ...
    async def test_cache_operations_performance(self, mock_cache):
        ...
        for i in range(doc_count):
            await mock_cache.set_document_metadata(i, {
                "title": f"Doc {i}",
                "size": 1024 + i,
                "created": "2024-01-01"
            })
        ...
        assert doc_count / write_time > 1000, f"Cache writes too slow: {doc_count/write_time:.1f} ops/sec"
        assert doc_count / read_time > 1000, f"Cache reads too slow: {doc_count/read_time:.1f} ops/sec"