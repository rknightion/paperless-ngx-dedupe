...
class TestDocumentsAPI:
    ...
    def test_list_documents_pagination(self, client, db_session):
        ...
        for i in range(15):
            doc = Document(paperless_id=i+1, title=f"Test Doc {i+1}", fingerprint=f"fp{i+1:03d}")
            db_session.add(doc)
        db_session.commit()
    ...
    def test_get_document_by_id(self, client, db_session):
        ...
        doc = Document(paperless_id=123, title="Specific Doc", fingerprint="specific123")
        db_session.add(doc)
        db_session.commit()
    ...
    def test_get_document_content(self, client, db_session):
        ...
        doc = Document(paperless_id=123, title="Doc with content", fingerprint="content123")
        db_session.add(doc)
        db_session.flush()
    ...
    def test_get_document_duplicates(self, client, db_session):
        ...
        # Add members
        member1 = DuplicateMember(group_id=group.id, document_id=doc1.id, is_primary=True)
        member2 = DuplicateMember(group_id=group.id, document_id=doc2.id, is_primary=False)
    ...
    async def test_sync_documents_success(self, mock_client_class, client, mock_cache):
        ...
        mock_client.get_document_content = AsyncMock(side_effect=lambda doc_id: f"Content for document {doc_id}")
...
class TestDuplicatesAPI:
    ...
    def test_delete_duplicate_group(self, client, db_session):
        ...
        deleted_group = db_session.query(DuplicateGroup).filter(DuplicateGroup.id == group_id).first()
    ...
    def test_get_duplicate_statistics(self, client, db_session):
        ...
        # Create test data
        for i in range(10):
            doc = Document(paperless_id=i+1, title=f"Doc {i+1}", fingerprint=f"fp{i+1}")
            db_session.add(doc)
        db_session.flush()
