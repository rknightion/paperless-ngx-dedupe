...
down_revision: str | Sequence[str] | None = "cf393bce516c"
...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_documents_processing_status_last_processed",
        "documents",
        ["processing_status", "last_processed"],
        unique=False,
    )
    op.create_index(
        "ix_documents_created_date_paperless_id",
        "documents",
        ["created_date", "paperless_id"],
        unique=False,
    )

    # Add indexes for duplicate groups table
    op.create_index(
        "ix_duplicate_groups_reviewed_resolved_confidence",
        "duplicate_groups",
        ["reviewed", "resolved", "confidence_score"],
        unique=False,
    )
    op.create_index(
        "ix_duplicate_groups_created_at",
        "duplicate_groups",
        ["created_at"],
        unique=False,
    )
    ...
    op.create_index(
        "ix_duplicate_members_group_id", "duplicate_members", ["group_id"], unique=False
    )
    op.create_index(
        "ix_duplicate_members_document_id",
        "duplicate_members",
        ["document_id"],
        unique=False,
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_documents_processing_status_last_processed", table_name="documents"
    )
