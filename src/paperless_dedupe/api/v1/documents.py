...
async def get_documents(
    ...
    search: str | None = Query(None, description="Search document titles"),
    ...
    if search:
        base_query = base_query.filter(Document.title.ilike(f"%{search}%"))
    if processing_status:
        base_query = base_query.filter(Document.processing_status == processing_status)
    ...
    if cursor:
        ...
        if order_desc:
            ...
            documents = (
                base_query.order_by(order_field.desc()).offset(skip).limit(limit).all()
            )
        else:
            order_field = getattr(Document, order_by, Document.id)
            documents = (
                base_query.order_by(order_field.asc()).offset(skip).limit(limit).all()
            )
...
async def get_document_stats(db: Session = Depends(get_db)):
    """Get basic document processing statistics (legacy endpoint)."""
    total = db.query(Document).count()
    pending = db.query(Document).filter(Document.processing_status == "pending").count()
    processing = (
        db.query(Document).filter(Document.processing_status == "processing").count()
    )
    completed = (
        db.query(Document).filter(Document.processing_status == "completed").count()
    )
    errors = db.query(Document).filter(Document.processing_status == "error").count()
...
async def get_sync_status(db: Session = Depends(get_db)):
    ...
    if not status_copy.get("status"):
        status_copy["status"] = (
            "processing" if status_copy.get("is_syncing") else "idle"
        )
    return status_copy
