...
async def broadcast_task_update(update: TaskUpdate):
    ...
    try:
        ...
        if update.step:
            ...
            if ("syncing" in step_lower or
                "sync" in step_lower and "analyzing" not in step_lower or
                "loading documents" in step_lower and "database" not in step_lower):
                is_sync_task = True
                message_type = "sync_update"
            # Processing/deduplication tasks
            elif ("analyzing" in step_lower or
                  "processing" in step_lower or
                  "deduplication" in step_lower or
                  "duplicate" in step_lower or
                  "loading document content" in step_lower):
                is_sync_task = False
                message_type = "processing_update"
        ...
        logger.debug(f"Broadcasted {message_type} for {update.task_id}: {update.status}")
